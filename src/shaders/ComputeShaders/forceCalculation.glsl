...
// Brute-force O(n^2) gravitational force calculation — GPU compute shader.
// Each invocation computes the net force on one particle by summing contributions
// from all other particles.  Plummer softening (squaredSoftening) prevents
// the force from diverging when two particles get very close.

layout( local_size_x = 64, local_size_y =1, local_size_z = 1  ) in;

uniform float squaredSoftening; // epsilon^2 added to r^2 before inversion
